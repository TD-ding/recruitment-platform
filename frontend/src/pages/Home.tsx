...
export default function Home() {
  ...
  useEffect(() => {
    api.get('/jobs', { params: { limit: 8 } }).then(res => setJobs(res.data)).catch(() => {});
  }, []);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    api.get('/jobs', { params: { keyword, limit: 20 } }).then(res => setJobs(res.data)).catch(() => {});
  };
    ...
    <div>
      ...
      <div className="bg-primary-600 text-white py-20">
        <div className="max-w-4xl mx-auto text-center px-4">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 max-w-2xl mx-auto">
            ...
            />
            <button type="submit" className="bg-primary-700 hover:bg-primary-800 px-6 py-3 rounded-lg font-medium">搜索</button>
          </form>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {jobs.map(job => (
            <Link key={job.id} to={`/jobs/${job.id}`} className="bg-white rounded-lg shadow-sm border p-5 hover:shadow-md transition-shadow">
